<?php
/* Welcome to itstar :)
This is the core itstar file where most of the
...
function itstar_scripts_and_styles() {
  ...
  if (!is_admin()) {
		...
		wp_register_style( 'itstar-rtl-stylesheet', get_stylesheet_directory_uri() . '/css/rtl.css', array('itstar-stylesheet'), '', 'all' );
		wp_register_style( 'itstar-ie-rtl-stylesheet', get_stylesheet_directory_uri() . '/css/ie/rtl.css', array('itstar-stylesheet'), '', 'all' );
    ...
    if ( is_singular() AND comments_open() AND (get_option('thread_comments') == 1)) {
		...
		wp_register_script( 'itstar-js', get_stylesheet_directory_uri() . '/js/scripts.js', array('jquery','scrolltofixed','onscreen'), '', true );
		...
		if(is_rtl()){
			wp_enqueue_style('itstar-rtl-stylesheet');
		}
		...
		if(preg_match('/(?i)msie [5-8]/',$_SERVER['HTTP_USER_AGENT'])){
			...
			if(is_rtl()){
				wp_enqueue_style('itstar-ie-rtl-stylesheet');
			}
...
// Related Posts Function (call using itstar_related_posts(); )
function itstar_related_posts() {
	echo '<ul id="itstar-related-posts">';
	global $post;
	...
	if($tags) {
		...
		if($related_posts) {
			...
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'itstar' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_postdata();
	echo '</ul>';
} /* end itstar related posts function */
...
add_action('wp_head','itstar_inline_style' );
function itstar_inline_style(){
